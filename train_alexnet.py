...
if __name__ == '__main__':
    ...
    nn.init.xavier_normal_(alexnet.classifier[6].weight)
    alexnet = alexnet.to(device)

    loss_fn = nn.CrossEntropyLoss()
    learning_rate = 1e-4
    optimizer = torch.optim.Adam(alexnet.parameters(), lr=learning_rate, weight_decay=1e-4)

    epochs = 100
    loss_ = 10
    save_root = "./log_data_alexnet/"
    writer = SummaryWriter('./runs/experiment_alexnet/')

    for t in range(epochs):
        print(f"Epoch {t + 1}\n-------------------------------")
        time_start = time.time()
        avg_loss, avg_acc = train(train_dataloader, alexnet, loss_fn, optimizer, device, writer, t)
        time_end = time.time()
        print(f"train time: {(time_end - time_start)}")
        val_accuracy, val_loss, val_f1 = validate(valid_dataloader, alexnet, loss_fn, device, writer, t)
        WriteData(save_root + "alexnet.txt",
                  "epoch", t,
                  "train_loss", avg_loss,
                  ...
                  "val_f1", val_f1)
        if t % 10 == 0:
            torch.save(alexnet.state_dict(), save_root + "alexnet_epoch" + str(t) + "_loss_" + str(avg_loss) + ".pth")
        torch.save(alexnet.state_dict(), save_root + "alexnet_last.pth")
        if avg_loss < loss_:
            loss_ = avg_loss
            torch.save(alexnet.state_dict(), save_root + "alexnet_best.pth")
